...
gcs_regions_target_object = f"chapter-4/regions/{logical_date_nodash}/regions.csv"
...
bq_temporary_table_id = f"{gcp_project_id}.{bq_temporary_extract_dataset_name}.{bq_temporary_extract_table_name}_{logical_date_nodash}"

gcs_trips_source_object = f"chapter-4/trips/{logical_date_nodash}/*.csv"
gcs_trips_source_uri = f"gs://{gcs_source_data_bucket}/{gcs_trips_source_object}"

bq_trips_table_name = "trips_partition"
...
# DWH
bq_fact_trips_daily_table_name = "facts_trips_daily_partition"
bq_fact_trips_daily_table_id = f"{gcp_project_id}.{bq_dwh_dataset}.{bq_fact_trips_daily_table_name}${logical_date_nodash}"
...
with DAG(
        ...
        configuration={
            "query": {
                ...
                "destinationTable": {
                    ...
                    "tableId": f"{bq_temporary_extract_table_name}_{logical_date_nodash}"
                },
        ...
        configuration={
            "query": {
                ...
                "destinationTable": {
                    ...
                    "tableId": f"{bq_fact_trips_daily_table_name}${logical_date_nodash}"
                },
                "createDisposition": 'CREATE_IF_NEEDED',
                "writeDisposition": 'WRITE_TRUNCATE',
                "timePartitioning": {
    ...
    bq_row_count_check_dwh_fact_trips_daily = BigQueryCheckOperator(
        ...
        sql=f"""
            ...
            WHERE trip_date = DATE('{logical_date}')
            """,
        use_legacy_sql=False,

        # Send signal for downstream dag
        outlets=[Dataset("dwh_fact_trips_daily")])
    ...
    bq_row_count_check_dwh_dim_stations = BigQueryCheckOperator(
        ...
        # Send signal for downstream dag
        outlets=[Dataset("dwh_dim_stations")]
    )
